# -*- coding: utf-8 -*-
"""
「Tigger」(https://tigger-esthe.com/schedule) の出勤データを取得するスクリプト。

・日付ごとに ?day=YYYY-MM-DD&from=YYYY-MM-DD というURLがあるので、1週間分アクセスして回る
・各ページから「セラピスト名」「ルーム（登戸・向ヶ丘遊園/溝の口/武蔵小杉/調布）」を抜き出す
・結果を data/tigger.json に保存する
...
SHOP_NAME = "Tigger"
REGION = "神奈川"
BASE_URL = "https://tigger-esthe.com/schedule"
OUTPUT_PATH = os.path.join(os.path.dirname(__file__), "data", "tigger.json")
...
ROOM_CANDIDATES = ["登戸・向ヶ丘遊園", "溝の口", "武蔵小杉", "調布"]
...
def fetch_day(date_str):
    ...
    for a in soup.find_all("a", href=True):
        ...
        path = href.replace("https://tigger-esthe.com", "")
