# -*- coding: utf-8 -*-
"""
「アマテラス横浜」(https://amaterasu-yokohama.com/schedule) の出勤データを取得するスクリプト。

・日付ごとに ?day=YYYY-MM-DD&from=YYYY-MM-DD というURLがあるので、1週間分アクセスして回る
・各ページから「セラピスト名」「ルーム（横浜/関内/武蔵小杉/厚木）」を抜き出す
・結果を data/amaterasu.json に保存する
...
SHOP_NAME = "アマテラス横浜"
REGION = "神奈川"
BASE_URL = "https://amaterasu-yokohama.com/schedule"
OUTPUT_PATH = os.path.join(os.path.dirname(__file__), "data", "amaterasu.json")
...
def fetch_day(date_str):
    ...
    for a in soup.find_all("a", href=True):
        ...
        path = href.replace("https://amaterasu-yokohama.com", "")
